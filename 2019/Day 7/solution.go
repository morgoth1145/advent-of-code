...
func part1(input string) {
	...
	for sequence := range helpers.PermuteInts(0, 1, 2, 3, 4) {
		...
		for _, phase := range sequence {
			lastAmplifierOutput = program.Clone().AsyncRun(intcode.InputChannelFunction(channeltypes.Chain(channeltypes.List(int64(phase)), lastAmplifierOutput), intcode.EOFPanic))
		}
