...
func part1(input string) {
	output := getDiagnostic(intcode.Parse(input).AsyncRun(channeltypes.List(int64(1))))
	println("The answer to part one is " + strconv.FormatInt(output, 10))
}

func part2(input string) {
	output := getDiagnostic(intcode.Parse(input).AsyncRun(channeltypes.List(int64(5))))
